...
export default async function FollowersPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const { data: profileRaw } = await supabase
    .from('profiles')
    .select('id, username, display_name, is_private, followers_count')
    .eq('username', username)
    .single()

  if (!profileRaw) notFound()
  const profile = profileRaw as Pick<Profile, 'id' | 'username' | 'display_name' | 'is_private' | 'followers_count'>
  ...
  if (canView) {
    ...
    if (ids.length > 0) {
      ...
      followers = (profilesRes.data ?? []) as ListUser[]
