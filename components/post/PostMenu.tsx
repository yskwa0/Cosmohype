...
export function PostMenu({ postId, postOwnerId, currentUserId }: {
  ...
  async function submitBlock() {
    await supabase.from('blocks').insert({ blocker_id: currentUserId, blocked_id: postOwnerId })
    setDoneMsg('ブロックしました。フィードを再読み込みすると反映されます。')
    setView('done')
  }
            ...
            {view === 'block-confirm' && (
              <div className="flex flex-col px-5 pb-8">
                ...
                <p className="text-sm mb-6" style={{ color: 'var(--text-muted)' }}>
                  ブロックすると、このユーザーの投稿がフィードに表示されなくなります。
                </p>
                <button
                  onClick={submitBlock}
